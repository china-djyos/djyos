...
{
	//已经在board.c里面初始化了SPI4的驱动
	 Spi *Reg;
	 Reg = (Spi *)ADS8688_SPI_BASE_ADDR;
	 //配置SPI使用GPIO引脚
	 RCC->APB2ENR |= RCC_APB2ENR_SPI4EN; // 使能SPI4
	 RCC->APB2RSTR |= RCC_APB2RSTR_SPI4RST; // 复位SPI4
	 RCC->APB2RSTR &= ~RCC_APB2RSTR_SPI4RST; // 停止复位SPI4
...
bool_t ADS8688_Init(u8 spi_port)
{
    ...
    //Channel 0~2 AUTO SEQ ENANBEL
    ret=__ADS8688_WriteReadProgramReg(AUTO_SEQ_EN,0x7);
    if(ret==false)
    	goto EXIT;
    //AD0~AD2三个通道测量范围为+-2.5*4.096V  LSB:312.5uv
    //Channel 3~4，测量范围设置为0~2.5Vref    LSB:156.25uv
    ret=__ADS8688_SetChannelInputRange(Channel_3_Input_Range,0x5);
    if(ret==false)
    	goto EXIT;
    ret=__ADS8688_SetChannelInputRange(Channel_4_Input_Range,0x5);
	if(ret==false)
		goto EXIT;

	//Channel 5~7 AUTO SEQ ENANBEL
//	ret=__ADS8688_WriteReadProgramReg(AUTO_SEQ_EN,0xE0);
//	if(ret==false)
//		goto EXIT;
	//Channel 5~7，测量范围设置为0~2.5Vref    LSB:156.25uv
	 ret=__ADS8688_SetChannelInputRange(Channel_5_Input_Range,0x5);
	 if(ret==false)
		goto EXIT;
...
bool_t ADS8688_GetValue(uint16_t *pAinBuf)
{
	...
	//首先为AUTO_RST模式下采样第0~2
	ret=__ADS8688_SetAutoScanSequence(0x7);
	if(ret==false)
		return false;
	__ADS8688_EnterAutoRstMode();
	ret=__ADS8688_GetRstModeData(AD_Buf,3);
	if(ret==false)
		return false;

	__ADS8688_MAN_Ch_Mode(MAN_Ch_3);
	dat=__ADS8688_GetManChModeData();
	gs_cp1_buf[CN_CP_SAM_RATE_BSP-1]=dat;
	dat=__GetPwmPeak(0);
	AD_Buf[3]=dat;

	__ADS8688_MAN_Ch_Mode(MAN_Ch_4);
	dat=__ADS8688_GetManChModeData();
	...
	//首先为AUTO_RST模式下采样第0~2
//	ret=__ADS8688_SetAutoScanSequence(0xE0);
//	if(ret==false)
//		return false;
//	__ADS8688_EnterAutoRstMode();
//	ret=__ADS8688_GetRstModeData(AD_Buf,3);
...
//		return false;
	...
	__ADS8688_MAN_Ch_Mode(MAN_Ch_5);
	dat=__ADS8688_GetManChModeData();
	AD_Buf[5]=dat;
