...
#include "blackbox.h.h"

//异常存储方案说明：
//1、使用板件上NANDFLASH最后1M用于存储异常记录信息；
...
#define RECORD_EXP_PAGE_SIZE       (2048+64)     //异常存储页大小
#define RECORD_EXP_START_PAGE      (65024)    //异常存储起始页
#define RECORD_EXP_END_PAGE        (65535)   //异常存储结束页
#define RECORD_EXP_START_BLOCK     (1016)    //异常存储起始block
#define RECORD_EXP_END_BLOCK       (1023)    //异常存储结束block
#define RECORD_EXP_BLOCK_NUM        (8)
...
enum EN_BlackBoxDealResult BrdExp_RecordSave(struct BlackBoxRecordPara  *recordpara)
{
    ...
    {
        return EN_BLACKBOX_RESULT_RECORD_NOSPACE;
    }
    ...
    Ret=stm32f7_PageProgram(CurRecMsg.RecordAddr,gs_expbuf,HW_ECC);
    if(Ret!=-2)
    {
        memcpy(&sLastRecMsg,&CurRecMsg,sizeof(tagExpRecMsg));
    }
    return EN_BLACKBOX_RESULT_SUCCESS;
}
...
{
    u32 loop;
    for(loop = RECORD_EXP_START_BLOCK;loop <= RECORD_EXP_END_BLOCK;loop++)
    {
    	stm32f7_BlockErase(loop);
    }
...
bool_t BrdExp_RecordCheckLen(u32 assignedno, u32 *recordlen)
{
    ...
    if( (NULL == recordlen) || (assignedno > sLastRecMsg.RecordNo)
    		||(assignedno >= RECODR_NO_MAX-1))
        return false;
    stm32f7_PageRead((RECORD_EXP_START_PAGE+assignedno-1),gs_expbuf,HW_ECC);
                            ...
                            struct BlackBoxRecordPara *recordpara)
{
   s32 Ret=0;
    ...
    Ret=stm32f7_PageRead((RECORD_EXP_START_PAGE+assignedno-1),gs_expbuf,HW_ECC);
    if(Ret==-2)
    {
    	printf("Read exp log form nandfalsh failed.\r\n");
    	memset(gs_expbuf,0,RECORD_EXP_PAGE_SIZE);
    	return false;
    }
...
void BrdExp_RecordScan(void)
{
    ...
    {
    	stm32f7_PageRead(RECORD_EXP_START_PAGE+i,gs_expbuf,HW_ECC);
...
bool_t ModuleInstall_BrdExp(ptu32_t para)
{
	...
	if(NandFlash_Ready())
	{
	    BlackBoxOpt.fnBlackBoxRecordScan      = BrdExp_RecordScan;
	    BlackBoxOpt.fnBlackBoxRecordSave      = BrdExp_RecordSave;
	    BlackBoxOpt.fnBlackBoxRecordClean     = BrdExp_RecordClean;
	    BlackBoxOpt.fnBlackBoxRecordGet       = BrdExp_RecordGet;
	    BlackBoxOpt.fnBlackBoxRecordCheckNum  = BrdExp_RecordCheckNum;
	    BlackBoxOpt.fnBlackBoxRecordCheckLen  = BrdExp_RecordCheckLen;
		...
		if(!BlackBox_RegisterRecorder(&BlackBoxOpt))
		{
	...
	{
		printf("Exp Storage media is not ready yet.\r\n");
        return false;
	}
}
