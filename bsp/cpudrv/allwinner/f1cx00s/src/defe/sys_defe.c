...
//  sysSetLocalInterrupt(ENABLE_IRQ);//开IRQ中断
    Int_Register(CN_INT_LINE_DE_FE);
    Int_SetClearType(CN_INT_LINE_DE_FE,CN_INT_CLEAR_AUTO);
    Int_IsrConnect(CN_INT_LINE_DE_FE,IRQ_DEFE_ISR);
    Int_SettoAsynSignal(CN_INT_LINE_DE_FE);
    Int_ClearLine(CN_INT_LINE_DE_FE);
    Int_RestoreAsynLine(CN_INT_LINE_DE_FE);
}
/*********************************************************************************************
DEFE配置
...
配置video_yuv422_to_argb
*/
extern unsigned char buff_y[1][CSI_W*CSI_H];
extern unsigned char buff_c[1][CSI_W*CSI_H];
void Defe_Config(int inmode,s32 monitorW,s32 monitorH)
{
    Defe_Config_video_uvcombined_yuv422_to_argb_x(CSI_W,CSI_H,monitorW,monitorH,(int)buff_y[0],(int)buff_c[0]);
}
/*********************************************************************************************
DEFE切换缓存
*********************************************************************************************/
...
{
    __scal_buf_addr_t src_buf_addr;
  src_buf_addr.ch0_addr=(int)buff_y[inx];
    src_buf_addr.ch1_addr=(int)buff_c[inx];
    src_buf_addr.ch2_addr=0;
