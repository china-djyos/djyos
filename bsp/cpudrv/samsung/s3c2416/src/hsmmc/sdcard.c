...
//%$#@describe      ****组件描述开始
//component name:"cpu driver hsmmc"//CPU的hsmmc驱动
...
//init time:medium                     //初始化时机，可选值：early，medium，later。
                                       //表示初始化时间，分别是早期、中期、后期
//dependence:"component file system","component heap","device file system"//该组件的依赖组件名（可以是none，表示无依赖组件），
                                       //选中该组件时，被依赖组件将强制选中，
                                       //如果依赖多个组件，则依次列出，用“,”分隔
...
#ifndef CFG_SDCARD_FSMOUNT_NAME   //****检查参数是否已经配置好
