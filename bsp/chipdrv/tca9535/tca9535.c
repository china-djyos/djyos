...
//%$#@describe      ****组件描述开始
//component name:"chip driver tca9535"//IIC接口的16口IO扩展
...
//init time:early               //初始化时机，可选值：early，medium，later。
                                //表示初始化时间，分别是早期、中期、后期
//dependence:"component lock","component iicbus","cpu driver iic"//该组件的依赖组件名（可以是none，表示无依赖组件），
                                //选中该组件时，被依赖组件将强制选中，
                                //如果依赖多个组件，则依次列出，用“,”分隔
...
#ifndef CFG_TCA9535_BUS_NAME   //****检查参数是否已经配置好
#warning    tca9535组件参数未配置，使用默认值
//%$#@num,0,100,
//%$#@enum,100000,400000,
