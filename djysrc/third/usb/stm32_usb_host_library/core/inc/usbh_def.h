...
#define  USB_H2D                                        0x00 //
#define  USB_D2H                                        0x80 // IN(Device to Host)
...
typedef struct 
{
  ...
  void*                pData;
  USBH_StatusTypeDef  ClassState; // CLASS的状态
} USBH_ClassTypeDef;
...
typedef struct _USBH_HandleTypeDef
{
  ...
  volatile HOST_StateTypeDef     resumeState;  // Host State Machine Value before into suspend or after resume
  ...
  volatile uint32_t         Timer; // SOF中断产生的计时
  //uint8_t               id;
  uint16_t               id; // 高8位用于表示hub，低8位用于host
  void*                 pData; // 指向寄存器
  void                  (* pUser )(struct _USBH_HandleTypeDef *pHandle, uint8_t id); // 用户注册的处理程序
  uint32_t              dwTimeout;
  uint32_t                (*pPipes)[15]; // hub分支上的设备，指向Pipes[15]
  USBH_StatusTypeDef    (*processSOF)(struct _USBH_HandleTypeDef *phost); // hub分支上的设备，处理SOF逻辑
#if (USBH_USE_OS == 1)
