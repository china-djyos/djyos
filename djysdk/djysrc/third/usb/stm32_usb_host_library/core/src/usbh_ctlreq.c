...
{ 
	...
	status = USBH_CtlReq(phost, buff , length );
	if(USBH_OK != status)
	{
		if(phost->dwTimeout++ > 10000)
		{
			...
			USBH_UsrLog("USB #%d stack: get description timeout, retry", phost->id);
		}
	}
	else
	{
                                   ...
                                   uint8_t DeviceAddress)
{
	...
	if(USBH_OK != status)
	{
		if(phost->dwTimeout++ > 10000)
		{
			phost->dwTimeout = 0;
			phost->RequestState = CMD_SEND;
			USBH_UsrLog("USB #%d stack: set address timeout, retry", phost->id);
		}
	}
	else
	{
                               ...
                               uint16_t cfg_idx)
{
	...
	if(USBH_OK != status)
	{
		if(phost->dwTimeout++ > 10000)
		{
			phost->dwTimeout = 0;
			phost->RequestState = CMD_SEND;
			USBH_UsrLog("USB #%d stack: set configuration timeout, retry", phost->id);
		}
	}
	else
	{
                        ...
                        uint8_t ep_num, uint8_t altSetting)
{
	...
	if(USBH_OK != status)
	{
		if(phost->dwTimeout++ > 10000)
		{
			phost->dwTimeout = 0;
			phost->RequestState = CMD_SEND;
			USBH_UsrLog("USB #%d stack: set interface timeout, retry", phost->id);
		}
	}
	else
	{
	...
	if(phost->RequestState == CMD_SEND)
	{
  	...
  	if(USBH_OK != status)
  	{
  		if(phost->dwTimeout++ > 10000)
  		{
  			phost->dwTimeout = 0;
  			phost->RequestState = CMD_SEND;
  			USBH_UsrLog("USB #%d stack: clear feature timeout, retry", phost->id);
  		}
  	}
  	else
  	{
  ...
  case CTRL_ERROR:
    ...
    */
    if (++ phost->Control.errorcount <= USBH_MAX_ERROR_COUNT)
    {
      ...
      // USBH_LL_Stop(phost); // TODO
      USBH_ErrLog("Control error, recover");
