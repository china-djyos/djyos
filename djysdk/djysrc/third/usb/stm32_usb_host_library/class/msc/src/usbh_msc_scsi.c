                                                ...
                                                uint8_t lun)
{
  ...
  {
  case BOT_CMD_SEND: // 建立数据格式
    
    /*Prepare the CBW and relevent field*/
    MSC_Handle->hbot.cbw.field.DataTransferLength = DATA_LEN_MODE_TEST_UNIT_READY; // 没有数据交互要求，直接获取
                                               ...
                                               SCSI_StdInquiryDataTypeDef *inquiry)
{
  ...
  if(USBH_OK != error)
  {
      if(phost->dwTimeout++ > 1000)
      {
          if(MSC_Handle->hbot.state == BOT_SEND_CBW_WAIT)
              MSC_Handle->hbot.cmd_state = BOT_CMD_SEND;
          phost->dwTimeout = 0;
          error = USBH_TIMEOUT;
      }
