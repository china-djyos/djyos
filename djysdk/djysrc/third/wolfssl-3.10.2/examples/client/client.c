...
//        }
        //zqf--TODO, we use buffer instead
        extern unsigned char clientcert[];
        extern unsigned int  clientcert_size;
        if (wolfSSL_CTX_use_certificate_chain_buffer(ctx, clientcert,clientcert_size)
                                                               != SSL_SUCCESS) {
            wolfSSL_CTX_free(ctx);
            err_sys("can't load client cert file, check file and run from"
                    " wolfSSL home dir");
        ...
        {
        	printf("load ca success!\n\r");
        }
        extern unsigned char clientkey[];
        extern unsigned int  clientkey_size;
        if (wolfSSL_CTX_use_PrivateKey_buffer(ctx, clientkey,clientkey_size,SSL_FILETYPE_PEM)
                                         != SSL_SUCCESS) {
            wolfSSL_CTX_free(ctx);
            err_sys("can't load client private key file, check file and run "
                    "from wolfSSL home dir");
    ...
    if (!usePsk && !useAnon) {
        ...
        extern unsigned int  cacert_size;
        if (wolfSSL_CTX_load_verify_buffer(ctx,cacert,cacert_size,SSL_FILETYPE_PEM)
                                                               != SSL_SUCCESS) {
            wolfSSL_CTX_free(ctx);
