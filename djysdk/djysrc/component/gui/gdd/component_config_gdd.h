...
//dependence:"gk","MsgQueue"         //该组件的依赖组件名（可以是none，表示无依赖组件），
                                             //选中该组件时，被依赖组件将强制选中，
                                             //如果依赖多个组件，则依次列出，用“,”分隔
...
#ifndef CFG_INPUTDEV_NUM        //****检查参数是否已经配置好
#warning    gdd组件参数未配置，使用默认值
//%$#@num,0,100,
#define CFG_INPUTDEV_NUM        1                   //"输入设备数量",输入设备（如触摸屏）的数量
#define CFG_DESKTOP_WIDTH       0                   //"桌面宽度",桌面尺寸（像素数）宽度，0=显示器宽度
#define CFG_FILL_COLOR          0xFF                //"填充色",创建桌面时的填充色，用888格式
//%$#@enum,true,false,
//%$#@string,1,10,
//%$#select,        ***定义无值的宏，仅用于第三方组件
//%$#@free,
#define CFG_INPUTDEV_NAME       ""      //"name",输入设备（如触摸屏）的名字
#define CFG_DISPLAY_NAME        ""      //"name",LCD显示器的名字
#define CFG_DESKTOP_NAME        ""      //"name",桌面窗口的名字
#define CFG_DESKTOP_HEIGHT      CN_COLOR_BLUE       //"桌面高度",桌面尺寸（像素数）高度，0=显示器高度
#define CFG_DESKTOP_FORMAT      CN_SYS_PF_RGB565    //"像素格式",桌面窗口像素格式，常数在gkernel.h中定义，一般使用与显示器相同颜色
#define CFG_GRAY_BASE_COLOR     CN_COLOR_WHITE      //"name",CFG_DESKTOP_FORMAT设为灰度时才需要设置
#endif
//%$#@end configue  ****参数配置结束
