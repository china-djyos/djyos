...
//dependence:"stm32f3"             //该组件的依赖组件名（可以是none，表示无依赖组件），
                                //选中该组件时，被依赖组件将强制选中，
                                //如果依赖多个组件，则依次列出，用“,”分隔
...
//内部会调用该量，因此需此处赋值
//例如ETH网络驱动中，stm32f3xx_hal_eth.c中会调用
uint32_t SystemCoreClock = CN_CFG_MCLK;         //HAL_RCC会用到此量,72M
...
void HAL_TickInit(void)
{
    ...
    TimHandle.Init.Prescaler     = uwPrescalerValue;     //分频系数
