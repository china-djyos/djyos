...
#define TCA9535_CLK_FRE             (100*1000)      //总线速度，单位Hz

#define TCA9535_ADDR (0x20) //这和板件硬件连接有关，到时候放到板件里面定义
...
bool_t TCA9535_Init(char *BusName)
{
    ...
    if(NULL != IIC_DevAdd_s(&Dev,BusName,DevName,TCA9535_ADDR,0,8))
    {
    	pg_TCA9535_Dev = &Dev;
    	IIC_BusCtrl(pg_TCA9535_Dev,CN_IIC_SET_CLK,TCA9535_CLK_FRE,0);//设置时钟大小
