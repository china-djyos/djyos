...
#include "320x480/0_16.h"
//#include "320x480/1_16.h"
//#include "320x480/2_16.h"
//#include "320x480/3_16.h"
//#include "320x480/4_16.h"
#include "320x480/5_16.h"
#include "320x480/6_16.h"
#include "320x480/7_16.h"
//#include "320x480/8_16.h"
#include "320x480/9_16.h"
const unsigned char *gpic[] = {code_pic0, code_pic5, code_pic6, code_pic7, code_pic9};
...
void Read_LCD_ID()
{
    ...
    unsigned char lcd_id[3] ={0x00, 0x00, 0x00};
    SendRecvDataSPI(&cmd, 1, lcd_id, 3);
}
...
void LCD_Init(void)
{
	...
	//ILI9488+CTC3.5
	//Read_LCD_ID();
//    unsigned char cmd =0x04;//0xD3;
//    unsigned char lcd_id[4] ={0x00, 0x00, 0x00, 00};
//    SendRecvDataSPI(&cmd, 1, lcd_id, 4);
...
//    unsigned char cmd =0xD3;
//    unsigned char lcd_id[4] ={0x00, 0x00, 0x00, 00};
//    SendRecvDataSPI(&cmd, 1, lcd_id, 4);
//    unsigned char xcmd =0x0C;
//    unsigned char xxxx[1] ={0x00};
//    SendRecvDataSPI(&xcmd, 1, xxxx, 1);
...
void testDot(unsigned int x, unsigned int y, unsigned int color)
{
    BlockWrite(x,x,y,y);
    WriteOneDot(color);
}
#define ROW_TEMP 200
#define COLO_DEP 2
unsigned char screen[ROW_TEMP][COL][COLO_DEP];
void DispColor(unsigned char color1,unsigned char color2,unsigned char color3)
{
	...
	WriteDataBytes(screen, (i%ROW_TEMP+1)*COL*COLO_DEP);
#else
...
//            screen[i%ROW_TEMP][j][2] = color3;
    ...
    WriteDataBytes(screen, (ROW%ROW_TEMP)*COL*COLO_DEP);
#endif
...
void DispColorXXX(unsigned int color)
{
 unsigned int i,j;
...
void DispPicCode(unsigned char *pic, unsigned len)
{
    int offset = 0;

    static unsigned int cnt = 0;
    pic = gpic[cnt%(sizeof(gpic)/sizeof(gpic[0]))];
    cnt++;
    ...
    for(i=0;i<ROW;i++)
    {
        ...
        for(j=0;j<COL;j++)
        {
            ...
            offset++;
//            screen[i%ROW_TEMP][j][2] = code_pic[offset];
//            offset++;
        }
        if ((i+1)%ROW_TEMP==0) {
            WriteDataBytes(screen, sizeof(screen));
...
void DispColorEx(unsigned int color)
{
 unsigned int i,j;
...
void DispBand(void)	 
{
	...
	for(i=0;i<8;i++)
	{
		for(j=0;j<ROW/8;j++)
		{
	        for(k=0;k<COL;k++)
			{
	            WriteDataBytes((unsigned char*)&color[i], 2);
				//WriteData(color[i]>>8);
				//WriteData(color[i]);
	...
	for(j=0;j<ROW%8;j++)
	{
        for(k=0;k<COL;k++)
		{
            WriteDataBytes((unsigned char*)&color[7], 2);
			//WriteData(color[7]>>8);
			//WriteData(/*DBL=*/color[7]);
