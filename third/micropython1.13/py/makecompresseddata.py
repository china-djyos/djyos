...
def word_compression(error_strings):
    ...
    for line in error_strings.keys():   #统计单词出现次数
        check_non_ascii(line)
    ...
    #top128是由元组组成的list，元组的第一个元素是已排序的单词，第二个元素是该单词出现次数。
    #排序规则：首要规则：(串长度+1)*(数量-1)，次要规则：字母顺序，共取128个元素。
    #可以这样说，长度越长、出现次数越多的排前面，相同则按字母顺序排。
    top128 = sorted(topn.items(), key=bytes_saved)[:128]
    index = [w for w, _ in top128]      #已排序的单词组成list
    index_lookup = {w: i for i, w in enumerate(index)}  #已排序的单词为 key 的字典，item值就是0~127的序数。
...
def main(collected_path, fn):
    ...
    with open(collected_path, "r") as f:    #找出所有行中最长的行。
        for line in f:
            line = line.strip()     #掐头去尾空格
            if not line:
                continue
            num_uses += 1           #统计非空行数
            error_strings[line] = None  #字典元素置空
            max_uncompressed_len = max(max_uncompressed_len, len(line)) #查找最长的行
