...
def qstr_escape(qst):
    def esc_char(m):
        ...
        try:
            ...
            name = "0x%02x" % c
        return "_" + name + "_"
    #调用esc_char把qst中字母数字下划线以外的字符替换成codepoint2name数组中指定的名字
...
#qstrs 是以 static_qstr_list 为索引的字典
#    for num in qstrs:
#        print(qstrs[num])
    ...
    for infile in infiles:
        with open(infile, "rt") as f:
            for line in f:
                line = line.strip()     #去掉行首行尾空格

                # is this a config line?
                match = re.match(r"^QCFG\((.+), (.+)\)", line)  #查找配置行
                if match:
                    value = match.group(2)
                ...
                # is this a QSTR line?
                match = re.match(r"^Q\((.*)\)$", line)  #找出 Q(...) 格式的行，提取括号内的内容到group(1)
                if not match:
                    continue
...
def do_work(infiles):
    ...
    # qcfgs中的内容是文件中QCFG( )的内容
    # qstrs中的内容与文件中Q(xxx)相关，是一个元组组成的数组，每个成员：
    # （序号，xxx中特殊字符替换后的字符串，xxx）
