...
static int op_get_packet_duration(const unsigned char *_data,int _len){
  ...
  frame_size=opus_packet_get_samples_per_frame(_data, SAMPLE_RATE/*48000*/);
...
static int op_make_decode_ready(OggOpusFile *_of){
  ...
  if(_of->od!=NULL&&_of->od_stream_count==stream_count
   ...
   sizeof(*head->mapping)*channel_count)==0){
    ...
    _of->od=opus_multistream_decoder_create(SAMPLE_RATE/*48000*/,channel_count,
...
static opus_int32 op_calc_bitrate(opus_int64 _bytes,ogg_int64_t _samples){
  ...
  if(OP_UNLIKELY(_bytes>(OP_INT64_MAX-(_samples>>1))/(SAMPLE_RATE/*48000*/*8))){
    ogg_int64_t den;
    if(OP_UNLIKELY(_bytes/(OP_INT32_MAX/(SAMPLE_RATE/*48000*/*8))>=_samples)){
      return OP_INT32_MAX;
    }
    den=_samples/(SAMPLE_RATE/*48000*/*8);
  ...
  return (opus_int32)OP_MIN((_bytes*SAMPLE_RATE/*48000*/*8+(_samples>>1))/_samples,
   OP_INT32_MAX);
}
    ...
    while(begin<end){
      ...
      if(page_offset<0){
        ...
        if(op_granpos_cmp(gp,_target_gp)<0){
          ...
          if(diff>SAMPLE_RATE/*48000*/)break;
