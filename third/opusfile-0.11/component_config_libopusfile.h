...
//%$#@describe      ****组件描述开始
//component name:"libopusfile"   //OPUS FILE库
...
//init time:none                //初始化时机，可选值：early，medium，later。
                                //表示初始化时间，分别是早期、中期、后期
//dependence:"libopus","libogg_opus"             //该组件的依赖组件名（可以是none，表示无依赖组件），
                                //选中该组件时，被依赖组件将强制选中，
                                //如果依赖多个组件，则依次列出
...
//%$#@configue      ****参数配置开始
#if ( CFG_MODULE_ENABLE_LIBOPUS_FILE == false )
//#warning  "libopusfile  组件参数未配置，使用默认配置"
//%$#@target = cmdline   //header = 生成头文件,cmdline = 命令行变量，DJYOS自有模块禁用
#define CFG_MODULE_ENABLE_LIBOPUS_FILE    false //如果勾选了本组件，将由DIDE在project_config.h或命令行中定义为true
//%$#@num,0,100
//%$#@enum,true,false
//%$#@string,1,16
//%$#@select
