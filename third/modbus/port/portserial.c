...
#endif
/* ----------------------- Static variables ---------------------------------*/
static u8 SerialTransStack[0x800];
...
/* ----------------------- static functions ---------------------------------*/
static ptu32_t SerialTrans(void);
        ...
        eMBParity eParity)
{
    ...
    EvttID = DJY_EvttRegist(EN_INDEPENDENCE, 200, 0, 1, (void *)SerialTrans, 
                                    SerialTransStack, sizeof(SerialTransStack), "slave trans");
    if (CN_EVTT_ID_INVALID == EvttID)
    {
        error_printf("modbus", "slave trans Evtt regist fail\r\n");
        return bStatus;
    }
