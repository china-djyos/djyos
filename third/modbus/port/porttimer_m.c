...
BOOL xMBMasterPortTimersInit(USHORT usTimeOut50us)
{
    ...
    if (Lock_SempCreate_s(&xMasterTimeoutLock, 1, 0, CN_BLOCK_PRIO, "master timeout"))
    {
        ...
        if (TimerHandle)
        {
            AT32Timer_Time2Counter(50 * usTimeOut50us, &usT35TimeOut);
            AT32Timer_Time2Counter(MB_MASTER_DELAY_MS_CONVERT * 1000, &usDelayTimeOut);
            AT32Timer_Time2Counter(MB_MASTER_TIMEOUT_MS_RESPOND * 1000, &usRespondTimeOut);
