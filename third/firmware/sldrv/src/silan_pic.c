...
int silan_pic_request(pic_irq_t id, int subid, void (*hdl)(int subid))
{
	...
	SL_LOG("Request irq. (id=%d, subid=%d, hdl=%p)", id, subid, hdl);
	...
	if ((id == PIC_IRQID_SPDIF) && (subid != PIC_SUBID_SOLO)) {
		...
		SL_DBGLOG("SPDIF subid = %d", subid);
		SL_DBGLOG("SPDIF hdl   = %p", spdif_hdl[subid]);
		SL_DBGLOG("hdl         = %p", hdl);

		if (!spdif_hdl[subid]) {
			err = pic_port_request(id, (void *)pic_hdl_spdif);
			if (err) {
		...
		SL_DBGLOG("SDMAC subid = %d", subid);
		SL_DBGLOG("SDMAC hdl   = %p", sdmac_hdl[subid]);
		SL_DBGLOG("hdl         = %p", hdl);

		if (!sdmac_hdl[subid]) {
			err = pic_port_request(id, (void *)pic_hdl_sdmac);
			if (err) {
				SL_DBGLOG("request irq failed. (pic port, %d)", err);
				return err;
			}
		...
		SL_DBGLOG("ADMAC subid = %d", subid);
		SL_DBGLOG("ADMAC hdl   = %p", admac_hdl[subid]);
		SL_DBGLOG("hdl         = %p", hdl);

		if (!admac_hdl[subid]) {
			err = pic_port_request(id, (void *)pic_hdl_admac);
			if (err) {
				SL_DBGLOG("request irq failed. (pic port, %d)", err);
				return err;
			}
		...
		SL_DBGLOG("GPIO1 subid = %d", subid);
		SL_DBGLOG("GPIO1 hdl   = %p", gpio1_hdl[subid]);
		SL_DBGLOG("hdl         = %p", hdl);

		if (!gpio1_hdl[subid]) {
			err = pic_port_request(id, (void *)pic_hdl_gpio1);
			if (err) {
				SL_DBGLOG("request irq failed. (pic port, %d)", err);
				return err;
			}
		...
		SL_DBGLOG("GPIO1 subid = %d", subid);
		SL_DBGLOG("GPIO1 hdl   = %p", gpio2_hdl[subid]);
		SL_DBGLOG("hdl         = %p", hdl);

		if (!gpio2_hdl[subid]) {
			err = pic_port_request(id, (void *)pic_hdl_gpio2);
			if (err) {
				SL_DBGLOG("request irq failed. (pic port, %d)", err);
				return err;
			}
		...
		SL_DBGLOG("TIMER subid = %d", subid);
		SL_DBGLOG("TIMER hdl   = %p", timer_hdl[subid]);
		SL_DBGLOG("hdl         = %p", hdl);
		if (!timer_hdl[subid]) {
			err = pic_port_request(id, (void *)pic_hdl_timer);
			if (err) {
				SL_DBGLOG("request irq failed. (pic port, %d)", err);
				return err;
			}
		...
		SL_DBGLOG("MISC  subid = %d", subid);
		SL_DBGLOG("MISC  hdl   = %p", misc_hdl[subid]);
		SL_DBGLOG("hdl         = %p", hdl);

		if (!misc_hdl[subid]) {
			err = pic_port_request(id, (void *)pic_hdl_misc);
			if (err) {
				SL_DBGLOG("request irq failed. (pic port, %d)", err);
				return err;
			}
		...
		SL_DBGLOG("PMU  subid = %d", subid);
		SL_DBGLOG("PMU  hdl   = %p", pmu_hdl[subid]);
		SL_DBGLOG("hdl         = %p", hdl);

		if (!pmu_hdl[subid]) {
			err = pic_port_request(id, (void *)pic_hdl_pmu);
			if (err) {
				SL_DBGLOG("request irq failed. (pic port, %d)", err);
				return err;
			}
