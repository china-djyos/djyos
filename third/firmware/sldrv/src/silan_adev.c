...
#else
static int			__buf[ADEV_MAX_DMAC_CHNL][ADEV_MAX_LLI_SIZE*ADEV_LLI_NUM/sizeof(int)]	__attribute__ ((section(".swap")));
static DMA_LLI		__dma_lli[ADEV_MAX_DMAC_CHNL][ADEV_LLI_NUM]						__attribute__ ((section(".swap")));
#endif
	...
	if (buf) {
		if(dma_dir == DMAC_DIR_M2P) {
			...
			if (done_lli->psrcaddr != __zero) {
				...
				done_lli->raw.srcaddr = (u32)ptr_cpu_to_dma((int *)__zero);
			...
			if (bytes == lli_size) {
				while (next_lli->pnextlli != done_lli->pnextlli) {
					if (next_lli->psrcaddr == __zero) {
						...
						next_lli->raw.srcaddr  = (u32)ptr_cpu_to_dma((int *)data);
						req->buf_pre_io_bytes += lli_size;
			...
			next_lli->raw.dstaddr = (u32)ptr_cpu_to_dma((int *)data);
			dcache_writeback(next_lli, sizeof(*next_lli));
...
ADEV_I2S_HANDLE *silan_adev_i2s_alloc(I2S_ID i2s)
{
	...
	pdev = (ADEV_I2S_HANDLE *)malloc(sizeof(ADEV_I2S_HANDLE));
	if(pdev == NULL) {
