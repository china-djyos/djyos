...
static void pmu_int_enable(u32 val)
{
	u32 tmp;
...
static void pmu_int_enable_1(u32 val)
{
	u32 tmp;
	sysclk_regs_protect_open();
...
static void pmu_int_disable(u32 val)
{
	u32 tmp;
	sysclk_regs_protect_open();
	tmp =  sysclk_st->sys_ctr1;
...
static void pmu_int_disable_1(u32 val)
{
	u32 tmp;
	sysclk_regs_protect_open();
	tmp =  sysclk_st->sys_ctr2;
...
void silan_clear_pmu_status(u32 val)
{
	sysclk_regs_protect_open();		
...
void silan_upll32k_sel(u8 clk_sel)
{
	u32 tmp;
	sysclk_regs_protect_open();
	tmp = sysclk_st->sys_ctr0;
...
void silan_pmu_irq_enable(u32 id)
{
	if(id == PMU_IRQ_WDT)
		pmu_int_enable_1(id);
	else
...
{
	u32 tmp,sysclk_val;
	u32 tmp_vco,tmp_RefDiv,tmp_div1,tmp_div2;
	tmp = sysclk_st->sys_ctr0;
	switch((tmp>>29)&0x3)
	{
		case 0:	//RCH
...
static void syspll_change(u8 sel)
{
	u32 tmp;	
	sysclk_regs_protect_open();
	switch(sel)
...
void silan_syspll_init(u32 sscg)
{
	u32 tmp;
	volatile int i = 0;
//	syspll_power_down();///hm modify for debug 20180503
...
void silan_sysclk_change(silan_sysclk_ref_t sysclk_ref,silan_syspllsel_t if_clk_sel)
{
	...
	if (sysclk_ref == SYSCLK_FROM_PLL)
		pre_sysclk_sel(SYSCLK_FROM_RCH, 0);
	ret = pre_sysclk_sel(sysclk_ref, if_clk_sel);
