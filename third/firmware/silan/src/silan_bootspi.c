...
static int spi_flash_read_u8(uint32_t addr, uint8_t *data, uint32_t num, uint8_t read_cmd,uint32_t fast_mode)
{
    unsigned int i;
    spi_flash_init(bootspi.clk,bootspi.cfg&BOOTSPI_CFG_PROREAD,bootspi.cfg&BOOTSPI_CFG_RCV_CPOL);
    spi_read_mode_sel(read_cmd,addr);
...
static int spi_flash_pagewrite_u8(uint8_t* pBuffer, uint32_t WriteAddr, uint32_t NumByteToWrite,uint32_t fast_mode)
{
	int g_result;
	unsigned int i;
	spi_flash_init(bootspi.clk,bootspi.cfg&BOOTSPI_CFG_PROREAD,bootspi.cfg&BOOTSPI_CFG_RCV_CPOL);
