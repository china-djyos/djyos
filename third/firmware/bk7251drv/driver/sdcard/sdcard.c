...
void sdcard_isr_initialize(void)
{
    SDIO_Error err = SD_OK;
...
right_return:
    ...
    {
        error_printf("sdcard","file systems mount fail\r\n");
        return ;
    }
    djy_gpio_irq_enable(GPIO12, 0);
    djy_gpio_attach_irq(GPIO12, PIN_IRQ_MODE_RISING, (void *)sdcard_uninitialize, 0);   //设置卡被拔掉时的中断响应函数
