...
beken_semaphore_t fed6_ind_sema = NULL;
...
ais_err_t feb3_send_fed6_ind_value(uint32_t len,uint8_t *buf,uint16_t seq_num)
{
    ...
    while(1)
    {
        ...
        {
            ret = rtos_get_semaphore(&fed6_ind_sema, SECONDS);
            if(0 != ret)
            {
                status = AIS_ERR_GATT_INDICATE_FAIL;
            }
        }
    }
...
ais_err_t feb3_send_fed8_ntf_value(uint32_t len,uint8_t *buf,uint16_t seq_num)
{
    ...
    {
        status = AIS_ERR_GATT_NOTIFY_FAIL;
    }
                                               ...
                                               kernel_task_id_t const src_id)
{
    ...
    if(fed6_ind_sema == NULL)
    {
        ret = rtos_init_semaphore(&fed6_ind_sema, 1);
        ASSERT(0 == ret);
    }
    ...
    if(param->ind_cfg != 0)
    {
        ...
        rtos_set_semaphore(&fed6_ind_sema);
    }
    else
    {
        ais_sdk_env.fed6_ind_send_allow = 0;
