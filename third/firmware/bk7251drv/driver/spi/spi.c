...
#define SPI_PERI_CLK_DCO        (180 * 1000 * 1000)
    ...
    if (max_hz > 4333000)
    {
//        printf("config spi clk source DCO\n");

        if(max_hz > 30000000) // 180M/2 / (2 + 1) = 30M
        {
            spi_clk = 30000000; 
            printf("input clk > 30MHz, set input clk = 30MHz\n");
        } else {
            spi_clk = max_hz;
        }
    ...
    {
        printf("config spi clk source 26MHz\n");
...
//    printf("spi_clk = %d \n", spi_clk);
//    printf("source_clk = %d \n", source_clk);
//    printf("target frequency = %d, actual frequency = %d \n", max_hz, source_clk / 2 / (div + 1));
}
