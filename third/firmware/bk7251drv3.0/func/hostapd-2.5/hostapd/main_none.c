...
enum hide_ssid g_SsidHideMode;      //add by lst for djyos
...
//add by lst for djyos
//-----------------------------------------------------------------------------
//功能：本机当热点时，设置AP隐藏模式，默认是不隐藏。本函数须在启动热点之前调用
//参数：mode，热点隐藏模式，参见类型定义
//返回：调用本函数之前的模式
//#include "ap/hostapd.h"
//-----------------------------------------------------------------------------
enum hide_ssid hostapd_set_hide_ssid_mode(enum hide_ssid mode)
{
    enum hide_ssid ret;
    ret = g_SsidHideMode;
    g_SsidHideMode = mode;
...
void hostapd_cfg_defaults_bss(struct hostapd_bss_config *bss)
{
    ...
    bss->ignore_broadcast_ssid = g_SsidHideMode;    //add by lst for djyos
    bss->eapol_version = 2;
