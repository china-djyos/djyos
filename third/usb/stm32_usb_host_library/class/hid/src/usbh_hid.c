...
extern USBH_StatusTypeDef USBH_HID_TouchDeInit(USBH_HandleTypeDef *phost);
...
{    
    ...
    {
        interface = USBH_FindInterface(phost, phost->pActiveClass->ClassCode, 0x0, 0x0);
        if(interface == 0xFF)
        {
    ...
    {
        HID_Handle->poll = HID_MIN_POLL; // 端点访问周期
    }
    ...
    phost->processSOF = USBH_HID_SOFProcess;
...
{   
    ...
    case HID_REQ_GET_REPORT_DESC:
        ...
        if (USBH_OK == classReqStatus)
        {
            ...
            HID_ReportParse(phost->device.Data, HID_Handle->HID_Desc.wItemLength);
    ...
    case HID_REQ_SET_IDLE:
        ...
        classReqStatus = USBH_HID_SetIdle(phost, 0, 0); // set Idle
    ...
    case HID_REQ_SET_PROTOCOL:
        ...
        classReqStatus = USBH_HID_SetProtocol (phost, 0);// set to boot protocol
        if(USBH_OK == classReqStatus)
        {
        ...
        else if(USBH_NOT_SUPPORTED == classReqStatus)
        {
            HID_Handle->ctl_state = HID_REQ_IDLE;
            phost->pUser(phost, HOST_USER_CLASS_ACTIVE);
            status = USBH_OK;
...
static USBH_StatusTypeDef USBH_HID_Process(USBH_HandleTypeDef *phost)
{
  USBH_StatusTypeDef status = USBH_OK;
    ...
    {
    case HID_INIT:
        HID_Handle->Init(phost); // 具体类型的初始化，例如touch
    ...
    case HID_IDLE:
        ...
        if(USBH_OK == status)
        {
            // fifo_write(&HID_Handle->fifo, HID_Handle->pData, HID_Handle->length); // 将数据抛弃掉
            HID_Handle->state = HID_SYNC; // 可以正常获取数据，进入同步状态
        }
    ...
    case HID_GET_DATA:
        ...
        HID_Handle->state = HID_POLL; // 不断的去轮询该地址
    ...
    case HID_POLL:
        ...
        else if(USBH_LL_GetURBState(phost , HID_Handle->InPipe) == USBH_URB_STALL) // TODO: IN Endpoint Stalled
        {
      
            // TODO: Issue Clear Feature on interrupt IN endpoint
...
static USBH_StatusTypeDef USBH_HID_SOFProcess(USBH_HandleTypeDef *phost)
{
    ...
    if(HID_Handle->state == HID_POLL)
    {
        ...
        {
            HID_Handle->state = HID_GET_DATA; // 轮询数据
            #if (USBH_USE_OS == 1)
            osMessagePut ( phost->os_event, USBH_URB_EVENT, 0);
            #endif
        }
...
static void  USBH_HID_ParseHIDDesc (HID_DescTypeDef *desc, uint8_t *buf)
{
    ...
    desc->wItemLength              =  LE16  (buf + 7); // report descriptor大小
} 
