...
USBH_StatusTypeDef USBH_MSC_BOT_REQ_Reset(USBH_HandleTypeDef *phost)
{
  ...
  phost->Control.setup.b.bmRequestType = USB_H2D | USB_REQ_TYPE_CLASS | \
                              USB_REQ_RECIPIENT_INTERFACE; // 'b0010_0001
...
USBH_StatusTypeDef USBH_MSC_BOT_Process (USBH_HandleTypeDef *phost, uint8_t lun)
{
  ...
  {
  case BOT_SEND_CBW: // 发送CBW
    MSC_Handle->hbot.cbw.field.LUN = lun;
    MSC_Handle->hbot.state = BOT_SEND_CBW_WAIT; // 进入等待
  ...
  case BOT_SEND_CBW_WAIT: // 等待接受
    
    URB_Status = USBH_LL_GetURBState(phost, MSC_Handle->OutPipe); 
    
    if(URB_Status == USBH_URB_DONE)
    { 
      if ( MSC_Handle->hbot.cbw.field.DataTransferLength != 0 ) // 主机需要交互数据
      {
      ...
      {/* If there is NO Data Transfer Stage */
        MSC_Handle->hbot.state = BOT_RECEIVE_CSW; // 没有数据交互
      }
#if (USBH_USE_OS == 1)
    ...
    {
      MSC_Handle->hbot.state  = BOT_ERROR_OUT; // BOT_SEND_CBW_WAIT
#if (USBH_USE_OS == 1)
    osMessagePut ( phost->os_event, USBH_URB_EVENT, 0);
#endif       
    }
    ...
    { // 超时机制
        ...
        {
            phost->dwTimeout = 0;
            MSC_Handle->hbot.state = BOT_SEND_CBW; // TODO
        }
  ...
  case BOT_DATA_IN_WAIT:  
    ...
    if(URB_Status == USBH_URB_DONE) 
    {
      ...
      else
      {
        ...
        MSC_Handle->hbot.state = BOT_RECEIVE_CSW; // 数据交互结束
#if (USBH_USE_OS == 1)
        osMessagePut ( phost->os_event, USBH_URB_EVENT, 0);
#endif 
      }
...

    {
        ...
        {
            phost->dwTimeout = 0;
            MSC_Handle->hbot.state = BOT_DATA_IN; // TODO
        }
  ...
  case BOT_DATA_OUT_WAIT:
...

    {
        ...
        {
            phost->dwTimeout = 0;
            MSC_Handle->hbot.state = BOT_DATA_OUT; // TODO
        }
  ...
  case BOT_RECEIVE_CSW_WAIT:
...

    {
        ...
        {
            phost->dwTimeout = 0;
            MSC_Handle->hbot.state = BOT_RECEIVE_CSW; // TODO
        }
