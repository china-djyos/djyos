...
/******************************************************************************
* 查找目标路径文件的目录
* 参数: startDir -- 起始目录对象，NULL则从根目录开始
*       path -- 查找的路径
*       name -- 返回值，最后一个节点名
*       symDepth --
*       notDir -- 返回值,"1"表示不是目录。
                         ...
                         int *notDir, int *loop)
{
    ...
    while (dir) {
        ...
        *name = restOfPath;
        ...
        while (*restOfPath && !yaffsfs_IsPathDivider(*restOfPath)) { // 获取路径中的某一个节点
        ...
        if (!*restOfPath)
            ...
            if (yaffs_strcmp(str, _Y(".")) == 0) {
                ...
                dir = yaffs_find_by_name(dir, str); // 查找节点

                dir = yaffsfs_FollowLink(dir, symDepth, loop); // 如果节点是link, 则找对应链接的节点
                        ...
                        int *notDir, int *loop)
{
     ...
     */
    if (dir && (yaffs_strcmp(name, _Y("..")) == 0)) { // 最后一个节点为'..'
        dir = dir->parent;
        obj = dir;
    } else if (dir && (yaffs_strcmp(name, _Y(".")) == 0)) // 最后一个节点为'.'
        obj = dir;
    else if (dir && *name) // 最后一个节点及其目录都存在
            ...
            int oflag, int mode, int sharing)
{
    ...
    handle = yaffsfs_NewHandleAndFileDes(); // 先分配句柄

    if (handle < 0) {
        yaffsfs_SetError(-ENFILE); // 文件句柄已分配完
        errorReported = 1;
    } else {

        fd = yaffsfs_HandleToFileDes(handle); // handle与fd对应

        /* try to find the exisiting object */
        obj = yaffsfs_FindObject(reldir, path, 0, 1, NULL, NULL, NULL);     //找path的obj
        ...
        if (obj) {
            ...
            if(is_dir) {
                dsc = yaffsfs_opendir_reldir_no_lock(reldir, path); // 打开次目录
                if (!dsc) {
                    openDenied = 1;
...
* 删除路径的目标文件
* 参数: path --
*      isDirectory -- 目录（1）；文件（0）；目录或者文件（3）；
* 返回: "-1" -- 错误,具体原因查询erron;
*      "0" -- 成功。
* 备注：修改了部分逻辑
**/
                ...
                const YCHAR *path, int isDirectory)
{
    ...
    obj = yaffsfs_FindObject(reldir, path, 0, 0, NULL, NULL, NULL); // 查找路径中的最后一个存在的对象
    dir = yaffsfs_FindDirectory(reldir, path, &name, 0, &notDir, &loop);

    if (!dir && notDir)
        yaffsfs_SetError(-ENOTDIR); // 不是目录
    else if (loop)
        yaffsfs_SetError(-ELOOP); // 存在环绕逻辑
    else if (!dir)
        yaffsfs_SetError(-ENOENT); // 不存在
    else if (yaffs_strncmp(name, _Y("."), 2) == 0)
        yaffsfs_SetError(-EINVAL);
    else if (!obj)
        yaffsfs_SetError(-ENOENT); // 目标节点不存在
    else if (obj->my_dev->read_only)
        yaffsfs_SetError(-EROFS); // 文件系统只读
    else if (!isDirectory &&
         obj->variant_type == YAFFS_OBJECT_TYPE_DIRECTORY)
        yaffsfs_SetError(-EISDIR); // 删除文件，但目标是目录
    else if ((1 == isDirectory) &&
         obj->variant_type != YAFFS_OBJECT_TYPE_DIRECTORY)
        yaffsfs_SetError(-ENOTDIR); // 删除目录，单目标不是目录
    else if ((1 == isDirectory) && obj == obj->my_dev->root_dir)
        yaffsfs_SetError(-EBUSY);   /* Can't rmdir a root */
    else {
        result = yaffs_unlinker(dir, name);

        if (result == YAFFS_FAIL && (1 == isDirectory))
            yaffsfs_SetError(-ENOTEMPTY); // 删除目录失败，目录非空。
    }
...
/* 删除文件或目录(修改) */
int __yaffs_unlink(const YCHAR *path)
{
    return yaffsfs_DoUnlink_reldir(NULL, path, 3);
}
...
int yaffs_rmdir_reldir(struct yaffs_obj *reldir, const YCHAR *path)
{
    ...
    if (yaffsfs_alt_dir_path(path, &alt_path) < 0) { // 为path分配一个更大的空间，为啥要这样？
        yaffsfs_SetError(-ENOMEM);
        return -1;
    }
    if (alt_path)
        path = alt_path;
    result = yaffsfs_DoUnlink_reldir(reldir, path, 1); // 1 表示删除的是目录
