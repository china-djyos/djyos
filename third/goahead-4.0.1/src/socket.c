...
#if defined(ME_COMPILER_HAS_SINGLE_STACK) || VXWORKS || __djyos__
    dual = 0;
#else
    dual = hasIPv6;
#endif
...
#endif
    enable = 1;
#if ME_UNIX_LIKE || VXWORKS || __djyos__
#if defined(__djyos__)
...
PUBLIC int socketSelect(int sid, int timeout)
{
    ...
    tv.tv_sec =(long)(timeout / 1000);
    tv.tv_usec =(unsigned long)(timeout % 1000) * 1000;
    ...
    //nEvents = select(socketHighestFd + 1,&readFds,&writeFds,&exceptFds,NULL);
    nEvents = select(socketHighestFd + 1,&readFds,NULL,NULL,NULL);
